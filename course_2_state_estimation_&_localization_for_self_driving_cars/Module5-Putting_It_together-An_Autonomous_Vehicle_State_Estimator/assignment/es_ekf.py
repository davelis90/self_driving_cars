...
################################################################################################
# var_imu_f = 0.10 # var_acc
# var_imu_w = 0.25 # var_gyro
# var_gnss  = 0.01
# var_lidar = 1.00
...
h_jac = np.zeros([3, 9]) # why 3x9 and not 3x10 ?????????? because of the error state (still unclear(
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    q_hat =  Quaternion(euler=delta_x_k[6:]).quat_mult_left(q_check) # axis_angle or euler ?????? left or right????
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t * v_est[k-1] + delta_t**2 / 2 * (Cns @ imu_f.data[k] + g)
    v_est[k] = v_est[k-1] + delta_t * (Cns @ imu_f.data[k] + g)
    q_est[k] = Quaternion(euler=imu_w.data[k-1]*delta_t).quat_mult_right(q_est[k-1]) # axis_angle or euler ?????? left or right????


    # 1.1 Linearize the motion model and compute Jacobians
    F_k_1 = np.eye(9)
    F_k_1 [0:3,3:6] = np.eye(3) * delta_t
    F_k_1 [3:6,6:9] = - skew_symmetric(Cns @ imu_f.data[k-1]) * delta_t # ?????????
    # print(F_k_1 [3:6,6:9])

    Q_k = delta_t**2 * np.diag([var_imu_f, var_imu_f, var_imu_f, var_imu_w, var_imu_w, var_imu_w])

    # 2. Propagate uncertainty
    p_cov[k] = F_k_1 @ p_cov[k-1] @ F_k_1.T + l_jac @ Q_k @ l_jac.T

    # 3. Check availability of GNSS and LIDAR measurements

    # Update states (save)
    if gnss_i < gnss.data.shape[0] and gnss.t[gnss_i] == imu_f.t[k-1]: # k or k-1 ??????
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov[k], gnss.data[gnss_i], p_est[k], v_est[k], q_est[k])
        gnss_i = gnss_i + 1
    if lidar_i < lidar.data.shape[0] and lidar.t[lidar_i] == imu_f.t[k-1]: # k or k-1 ??????
...
titles = ['Easting (x)', 'Northing (y)', 'Up (z)', 'Roll', 'Pitch', 'Yaw']
for i in range(3):
